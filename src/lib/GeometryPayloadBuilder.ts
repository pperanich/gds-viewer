	...
	for (const [sourceKey, layer] of documentLayerMap) {
		...
		const color =
			...
			(classification.isAnnotation
				? "#333333"
				: (config.defaultColor ??
					generateLayerColor(layer.layer, layer.datatype)));
		results.push({
			renderKey: sourceKey,
			sourceKey,
			layer: layer.layer,
...
export function createDefaultLayerStack(
	...
	for (const layer of document.layers.values()) {
		...
		const thickness = getDefaultThicknessForType(
			classification.type,
			defaultThickness,
		);
		const color =
...
export function estimateGeometryComplexity(
	...
	"chosenMode" | "modeReason" | "exceedsHardLimit"
> {
	let polygonCount = 0;
	let pointCount = 0;
...
> {
	...
	const hardLimitPointCount =
		options.hardLimitPointCount ?? DEFAULT_HARD_LIMIT_POINT_COUNT;
	const exceedsHardLimit =
		stats.estimated3DBytes >= hardLimitBytes ||
		stats.pointCount >= hardLimitPointCount;
	...
	const threshold =
		options.flatModeThresholdBytes ?? DEFAULT_FLAT_THRESHOLD_BYTES;
	return {
...
export function buildGeometryPayload(
	...
	const resolvedRenderEntries = resolveRenderEntries(
		document.layers.entries(),
		config,
	);
	...
	for (const cell of document.cells.values()) {
		for (const polygon of cell.polygons) {
			...
			if (processedPolygons === 1 || processedPolygons % 2000 === 0) {
				const progress =
					base + Math.floor((processedPolygons / totalPolygons) * span * 0.8);
				report(
					options,
	...
	report(
		options,
		base + Math.floor(span * 0.85),
		"Finalizing geometry payloads",
	);
	for (let i = 0; i < sortedLayers.length; i++) {
		...
		const opacity =
			data.entry.material?.opacity ?? data.classification.defaultOpacity;
	...
	return {
		...
		buildableRenderKeys,
		deferredRenderKeys: deferredRenderKeys.filter((key) =>
			buildableRenderKeys.includes(key),
		),
	};
}
