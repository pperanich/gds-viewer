...
export class MeasurementTool {
	...
	private onDocumentMouseMove = (e: MouseEvent) => {
		...
		if (this.dragMode.type === "label" && this.dragStartOffset) {
			...
			this.dragMode.measurement.labelScale = Math.max(
				0.25,
				Math.min(4, this.dragStartScale + delta),
			);
			this.updateLabelPosition(this.dragMode.measurement);
		}
	};
	...
	private onMouseMove = (e: MouseEvent) => {
		...
		if (!this.isMeasuring || !this.currentStart || !this.activeMeasurement)
			return;
	...
	private onMouseUp = (e: MouseEvent) => {
		...
		if (this.activeMeasurement) {
			const dist = this.activeMeasurement.startPoint.distanceTo(
	...
	private setHandlesVisible(labelDiv: HTMLDivElement, visible: boolean) {
		const handles = labelDiv.querySelectorAll(
			".scale-handle, .rotate-handle",
		) as NodeListOf<HTMLElement>;
		handles.forEach((h) => (h.style.display = visible ? "block" : "none"));
	}

	private getEndpointAtScreenPos(
		clientX: number,
		clientY: number,
	): { measurement: Measurement; which: "start" | "end" } | null {
		...
		for (const m of this.measurements) {
			...
			if (
				Math.hypot(mx - startScreen.x, my - startScreen.y) <
				ENDPOINT_HIT_RADIUS_PX
			) {
				return { measurement: m, which: "start" };
			}
			if (
				Math.hypot(mx - endScreen.x, my - endScreen.y) < ENDPOINT_HIT_RADIUS_PX
			) {
	...
	private getLabelScreenCenter(measurement: Measurement): {
		x: number;
		y: number;
	} {
		...
		const labelText = measurement.labelDiv.querySelector(
			".label-text",
		) as HTMLElement;
		if (!labelText) {
			return { x: 0, y: 0 };
	...
	private rebuildMeasurementLines(measurement: Measurement) {
		...
		measurement.lines = this.createMeasurementLines(
	...
	private updateLabelPosition(measurement: Measurement) {
		...
		const labelContent = measurement.labelDiv.querySelector(
			".label-content",
		) as HTMLElement;
    ...
    `;
		...
		const scaleSvg = document.createElementNS(
	...
	private applyStylesToLabel(labelDiv: HTMLDivElement) {
		...
		const scaleHandle = labelDiv.querySelector(
			".scale-handle svg path",
		) as SVGPathElement;
		if (scaleHandle) {
			scaleHandle.setAttribute("stroke", this.styles.color);
		}

		const rotateHandle = labelDiv.querySelector(
			".rotate-handle",
		) as HTMLElement;
		if (rotateHandle) {
