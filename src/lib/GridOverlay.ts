...
const pristineGrid = Fn(
	([uv, lineWidthPx, gridDiv, pixelRatio]: [
		...
		const grid2Raw = smoothstep(
			drawWidth.add(lineAA),
			drawWidth.sub(lineAA),
			gridUV,
		);
		const grid2Scaled = grid2Raw.mul(
			clamp(targetWidth.div(drawWidth), vec2(0.0), vec2(1.0)),
		);
		const grid2 = mix(
			grid2Scaled,
			targetWidth,
			clamp(uvDeriv.mul(2.0).sub(1.0), vec2(0.0), vec2(1.0)),
		);
...
function createInfiniteGridGeometry(): THREE.BufferGeometry {
	...
	const indices = new Uint16Array([
		0, 1, 2, 2, 1, 3, 4, 5, 0, 0, 5, 1, 5, 7, 1, 1, 7, 3, 7, 6, 3, 3, 6, 2, 6,
		4, 2, 2, 4, 0,
	]);
	...
	constructor() {
		const makeGridShader = (
			...
			Fn(() => {
				...
				const cameraCenteringOffset = floor(
					cameraPosition.xy.div(majorGridSize),
				).mul(majorGridSize);
				const worldPos = positionWorld.xy;
				const uv = worldPos.sub(cameraCenteringOffset);

				const majorAlpha = pristineGrid(
					uv,
					majorLineWidthPx,
					majorGridSize,
					pixelRatio,
