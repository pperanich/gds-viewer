...
function polygonToRing(polygon: Polygon, dbToUm: number): Ring {
	return polygon.points.map(
		(p) => [p.x * dbToUm, p.y * dbToUm] as [number, number],
	);
}
...
function getCadRings(context: BuildContext, cadId: string): Ring[] {
...
function resolveOverlayBufferUmLeaf(
	...
	if (!rule) {
		warnings.push(
			`Unknown alignment rule "${alignmentId}" referenced by mask CAD "${mask.cad}".`,
		);
		return 0;
	}
	const overlay =
		overlayMode === "max"
			? (rule.maxOverlay ?? rule.typicalOverlay)
			: rule.typicalOverlay;
...
function evaluateMaskRegion(
	...
	if (typeof mask !== "object" || mask === null) {
		...
		const buffered =
			bufferUm > 0 ? PolygonOps.bufferApprox(base, bufferUm) : base;
		result = {
		...
		result = {
			region,
			primaryCadId:
				evaluatedArgs.find((entry) => entry.primaryCadId)?.primaryCadId ?? null,
		};
	} else {
...
function getDieAreaRegion(context: BuildContext): {
	cadId: string;
	region: Region;
} {
	const cadId =
...
function buildDepositSolids(
	...
	for (const step of steps) {
		...
		if (step.pattern !== undefined) {
			...
			if (PolygonOps.isEmpty(region)) {
				context.warnings.push(
					`No polygons found for deposit mask (step ${step.id}).`,
				);
			}
		}
		...
		const group =
			selections.stepToGroup.get(step.id) ??
			selections.materialToGroup.get(step.material);
		solids.push({
			id: `step:${step.id}`,
			name: step.id,
...
function applyEtchSteps(
	...
	for (const step of steps) {
		...
		if (step.stopOn?.material) {
			...
			const stopSolids = current.filter(
				(solid) => solid.material === stopMaterial,
			);
			const stopRegion = PolygonOps.unionMany(
				stopSolids.map((solid) => solid.region),
			);
		...
		const nonTargets = current.filter(
			(solid) => solid.material !== step.targetMaterial,
		);
...
function solidsToLayerStack(
	...
	for (const solid of solids) {
		const cad =
			schema.cadLayers[solid.sourceCadId] ?? schema.cadLayers[dieAreaCadId];
...
export function buildDerivedGeometryPayload(
	...
	const solidsForRender = etchedSolids.filter((solid) =>
		shouldIncludeSolid(solid, selections),
	);
	const layerStack = solidsToLayerStack(
		schema,
		solidsForRender,
		dieArea.cadId,
		zUnits,
