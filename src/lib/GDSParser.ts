...
function* parseGDSWithDeprecatedRecords(
  ...
  while (offset < fileData.length) {
    ...
    if (tag === BGNEXTN || tag === ENDEXTN) {
      const value = dataLength === 4 ? dataView.getInt32(offset + 4, false) : null;
    ...
    switch (tag) {
      ...
      case RecordType.STRING: {
        ...
        data = textDecoder.decode(new Uint8Array(fileData.buffer, fileData.byteOffset + offset + 4, len));
        break;
      }
...
export async function parseGDSII(
  ...
  return new Promise((resolve, reject) => {
    ...
    worker.onmessage = (e: MessageEvent) => {
      const { type, progress, message, document, error } = e.data;
      
      if (type === "progress") {
        onProgress?.(progress, message);
      } else if (type === "complete") {
        worker.terminate();
        resolve(deserializeDocument(document));
      } else if (type === "error") {
        worker.terminate();
...
export async function parseGDSIISync(
  fileBuffer: ArrayBuffer,
  onProgress?: ParseProgressCallback
): Promise<GDSDocument> {
  onProgress?.(10, "Preparing file data...");
  ...
  try {
    ...
    if (error instanceof GDSParseError && error.message.includes("Unknown record type")) {
      records = [];
    ...
    switch (tag) {
      ...
      case RecordType.UNITS:
        if (
          ...
          units = { database: unitsData.metersPerUnit, user: unitsData.userUnit };
        }
        break;
      ...
      case RecordType.XY:
        if (currentPolygon && Array.isArray(data)) {
          ...
          const coord = data[0];
          if (Array.isArray(coord) && coord.length >= 2) {
            currentText.position = { x: coord[0] as number, y: coord[1] as number };
          }
        }
        break;
      ...
      case RecordType.ENDEL:
        if (currentPolygon && currentCell && currentPolygon.points) {
          ...
          if (currentPath.layer === undefined) currentPath.layer = currentLayer || 0;
          if (currentPath.datatype === undefined) currentPath.datatype = currentDatatype || 0;
          if (currentPath.width === undefined) currentPath.width = currentPathWidth || 0;
          if (currentPath.pathtype === undefined) currentPath.pathtype = currentPathType || 0;
          ...
          if (currentBox.layer === undefined) currentBox.layer = currentLayer || 0;
          if (currentBox.datatype === undefined) currentBox.datatype = currentDatatype || 0;
          ...
          if (currentText.layer === undefined) currentText.layer = currentLayer || 0;
